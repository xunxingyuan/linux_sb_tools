...
Chrome Manifest V3 扩展，提供称号统计、分阶段熔铸、Cloudflare R2 图片上传和可选广告隐藏。当前版本：**0.10.2**。运行时使用原生 JavaScript，不需要构建。
...
- **今日免费一抽**：识别免费一抽文案及明确的零积分费用。免费抽取不受最低保留积分限制，仍保留账号校验和用户配置的抽取确认；未标明费用的普通按钮不会被视为免费。
- **抽取前校验**：兼容按钮费用属性和“90 积分”等按钮文字。抽取前单独核对登录账号与最新余额，首次安装不依赖库存或历史同步；确认提交时再次校验，保留实际点击的单抽／十连／百连选项。无法识别账号、余额或费用时分别提示具体原因。
