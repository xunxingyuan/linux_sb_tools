(function attachParser(global) {
  ...
  function parseDrawAction(form, submitter) {
    ...
    const attributeCost = Number(button.getAttribute('data-cost'));
    const displayedCosts = [...label.matchAll(/(?<![\d.,+\-])([\d,]+(?:\.\d+)?)\s*积分/g)]
      .map(match => Number(match[1].replace(/,/g, '')));
    const costs = new Set(displayedCosts);
    if (Number.isFinite(attributeCost) && attributeCost > 0) costs.add(attributeCost);
    const [cost] = costs;
