(function attachParser(global) {
  ...
  function parseDrawAction(form, submitter) {
    if (!form) return null;
    const isSubmitControl = node => node?.form === form && (
      (node.tagName === 'BUTTON' && node.type === 'submit')
      || (node.tagName === 'INPUT' && node.type === 'submit')
    );
    if (submitter && !isSubmitControl(submitter)) return null;
    const controls = Array.from(form.elements || []).filter(isSubmitControl);
    const button = submitter || (controls.length === 1 ? controls[0] : null);
    if (!button) return null;

    const label = button.tagName === 'INPUT' ? button.value.trim() : textOf(button);
    const rawCost = button.getAttribute('data-cost');
    const attributeCost = rawCost !== null && rawCost.trim() !== '' ? Number(rawCost) : null;
    const displayedCosts = [...label.matchAll(/(?<![\d.,+\-])([+\-]?\s*\d[\d,]*(?:\.\d+)?)\s*积分/g)]
      .map(match => Number(match[1].replace(/[,\s]/g, '')));
    const costs = new Set(displayedCosts);
    if (attributeCost !== null) {
      if (!Number.isFinite(attributeCost) || attributeCost < 0) return null;
      costs.add(attributeCost);
    }
    if (/^(?:今日|每日)?\s*免费\s*(?:一|1)\s*抽(?:\s|[（(]|$)/.test(label)) costs.add(0);
    const [cost] = costs;
    if (costs.size !== 1 || !Number.isFinite(cost) || cost < 0) return null;
    return { button, cost, label: label || '抽取' };
  }
